...
// Ceiling Fans
var esCeilingFans = "http://www.homedepot.com/b/Lighting-Ceiling-Fans/N-5yc1vZbvn5/Ntk-All/Ntt-energy%2Bstar%2Bceiling%2Bfans?Ntx=mode+matchall&NCNI-5"
var baseCeilingFans = "http://www.homedepot.com/b/Lighting-Ceiling-Fans-Ceiling-Fans-Accessories-Ceiling-Fans/N-5yc1vZbvlq?Ns=P_REP_PRC_MODE%7C0"
...
function getAll(){
	async.eachSeries(typeArr, function(typeItem, bigCallback){
		...
		var filename = "ESAppliancesMore2.csv";
	  	csv()
		.from(esCsvString)
		.to('./' + filename)
		.on('end', function() {
	  ...
	  uri: currentUrl,
		...
		var seen = {};
	  	$(".product").each(function() {
			...
			var priceStrNoCommas = priceStrNoDollarSign.replace(",", "")
			var price = parseFloat(priceStrNoCommas);
			...
			var energyStarWhenItShouldntBe = false;
			if(currentUrl === currentTypeItem.base && title.attr('href').indexOf("Energy-Star") != -1 ) energyStarWhenItShouldntBe = true;

			if((titleStr.indexOf("DISCONTINUED") === -1) && !energyStarWhenItShouldntBe){
