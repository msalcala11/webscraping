...
var typeArr = [
	{name: 'ENERGY STAR Holiday Lights', start: 1, stop: 1},
];
...
async.eachSeries(typeArr, function(typeItem, bigCallback){
	...
	async.eachSeries(currentWattageArr, function(currentWattage, callback) {
		console.log("Scraping LED Holiday Lights")

		page = "http://www.homedepot.com/b/Decor-Holiday-Decorations-Christmas-Lights/Incandescent/N-5yc1vZc3tbZ1z11erb/Ntk-All/Ntt-holiday%2Blights?Ntx=mode%20matchall&NCNI-5&Ns=P_REP_PRC_MODE%7C0";
	...
	var filename = "homedepotHolidayLightsBaseline.csv";
  	csv()
	.from(csvString)
	.to('./' + filename)
	.on('end', function() {
	  ...
	  uri: page,
		...
		bulbArr = [];
	  	...
	  	$(".product").each(function() {
			...
			if((titleStr.indexOf("DISCONTINUED") === -1) && (titleStr.indexOf("100-Light") != -1)){
				...
				if(csvString.indexOf(bulb.link) === -1) {
					//seen[titleStr] = true;
