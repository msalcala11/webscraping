...
var typeArr = [
	{name: 'ENERGY STAR LED Bulbs  - 40W Equivalent', start: 40, stop:40},
	{name: 'ENERGY STAR LED Bulbs  - 60W Equivalent', start: 60, stop: 60},
	{name: 'ENERGY STAR LED Bulbs  - 75W Equivalent', start: 75, stop: 75},
	{name: 'ENERGY STAR LED Bulbs  - 100W Equivalent', start: 100, stop: 100}
];
...
async.eachSeries(typeArr, function(typeItem, bigCallback){
	...
	async.eachSeries(currentWattageArr, function(currentWattage, callback) {
		console.log("Scraping " + currentWattage + " Watt LEDs")
		...
		page = "http://www.homedepot.com/b/ENERGY-STAR-Certified/N-5yc1vZ1z0tlzw/Ntk-All/Ntt-" + currentWattage + "W%2Bled%2Bbulbs?Ntx=mode%2Bmatchall&NCNI-5&Ns=P_REP_PRC_MODE%7C0"
  	...
  	csv()
	.from(csvString)
	.to('./homedepotLEDs.csv')
	.on('end', function() {
    	console.log('Wrote to homedepotLEDs.csv');
  	})
})
	  ...
	  uri: page,
		...
		bulbArr = [];
	  	$(".product").each(function() {
			...
			// We want the price per bulb, so if the price is per pack we need to calculate the price per each bulb in the pack
			...
			if(price <= 999999 && (titleStr.indexOf("DISCONTINUED") === -1) && (titleStr.indexOf("CFL") === -1)){
