...
async.eachSeries(typeArr, function(typeItem, bigCallback){
	...
	async.eachSeries(currentWattageArr, function(currentWattage, callback) {
		console.log("Scraping Electric Clothes Dryers with Moisture Sensor")

		page = "http://www.homedepot.com/b/Appliances-Washers-Dryers-Dryers-Electric-Dryers/Moisture-Sensor/N-5yc1vZc3q1Z1z0v5jf?Ns=P_REP_PRC_MODE%7C0";
	...
	var filename = "homedepotElectricClothesDryerwithMoistureSensor.csv";
  	csv()
	.from(csvString)
	.to('./' + filename)
	.on('end', function() {
