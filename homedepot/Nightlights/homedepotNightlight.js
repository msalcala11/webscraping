...
var typeArr = [
	{name: 'Electroluminescent Nightlight', start: 7, stop: 7},
];
...
async.eachSeries(typeArr, function(typeItem, bigCallback){
	...
	async.eachSeries(currentWattageArr, function(currentWattage, callback) {
		console.log("Scraping " + currentWattage + " Watt Incandescent Nightlights")
	...
	var filename = "homedepotElectroluminescentNightlightBaseline.csv";
  	csv()
	.from(csvString)
	.to('./' + filename)
	.on('end', function() {
	  ...
	  uri: page,
		...
		bulbArr = [];
	  	$(".product").each(function() {
			...
			if(price <= 999999 && (titleStr.search("DISCONTINUED") === -1) && (titleStr.search("7-Watt") != -1)){
