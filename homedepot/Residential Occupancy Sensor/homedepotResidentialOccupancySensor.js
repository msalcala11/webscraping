...
var typeArr = [
	{name: 'Residential Occupancy Sensor', start: 1, stop: 1},
];
...
async.eachSeries(typeArr, function(typeItem, bigCallback){
	...
	async.eachSeries(currentWattageArr, function(currentWattage, callback) {
		console.log("Scraping Residential Occupancy Sensors")

		page = "http://www.homedepot.com/b/Electrical/N-5yc1vZarcd/Ntk-Extended/Ntt-residential%2Boccupancy%2Bsensor?Ns=P_REP_PRC_MODE%7C0&NCNI-5";
	...
	var filename = "homedepotResidentialOccupancySensor.csv";
  	csv()
	.from(csvString)
	.to('./' + filename)
	.on('end', function() {
